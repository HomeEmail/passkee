...
module.exports = {
	read(testPath) {
		...
		files.forEach((filePath) => {
			const filePaths = filePath.replace(testPath, '').replace(/\\/g, '/').replace(/^\//, '');
			...
			paths.forEach((it, index) => {
				...
				if (!target) {
					...
					target = {
						...
						children: [],
						isFile,
						path: path.join(testPath, paths.slice(0, index + 1).join('/'))
					};
					if (isFile) {
...
function readFile(file) {
	...
	res = res.map((it) => it.replace(/it\s?\(\s?[\'\"\`]/g, '').replace(/[\'\"\`]\,\s/g, ''));
